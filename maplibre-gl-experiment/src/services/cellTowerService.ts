...
export async function fetchCellTowersInArea(
   ...
   if (!apiKey) {
      console.warn(
         'OpenCelliD API key not configured. Please add VITE_OPENCELLID_API_KEY to your .env file'
      );
      return [];
   }
 ...
 * @returns BoundingBox
 */
export function calculateBoundingBox(lat: number, lon: number, radiusKm: number = 10): BoundingBox {
